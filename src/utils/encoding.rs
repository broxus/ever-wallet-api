...
pub fn encrypt_private_key(private_key: &[u8], key: [u8; 32], uuid: &uuid::Uuid) -> Result<String> {
    use chacha20poly1305::aead::NewAead;
    let nonce = Nonce::from_slice(&uuid.as_bytes()[0..12]);
...
pub fn decrypt_private_key(private_key: &str, key: [u8; 32], uuid: &uuid::Uuid) -> Result<Vec<u8>> {
    use chacha20poly1305::aead::NewAead;
    let nonce = Nonce::from_slice(&uuid.as_bytes()[0..12]);
    let key = chacha20poly1305::Key::from_slice(&key[..]);
