...
pub fn prepare_transfer(
    ...
    if init_data.data.values().count() >= 500_usize {
...
impl InitData {
    ...
    pub fn make_transfer_payload(
        ...
        messages.store_into(&mut message_builder, Cell::empty_context())?;

        let messages_cell = message_builder.clone().build()?;
...
impl TryFrom<&Cell> for InitData {
    ...
    fn try_from(data: &Cell) -> Result<Self, Self::Error> {
        ...
        let data = Dict::<u64, Cell>::load_from(&mut slice)?;
